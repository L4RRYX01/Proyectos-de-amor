...
function showDedicationText() { //seguidores
  ...
  if (!text) {
    text = `Para mi Princesa:\n\nHola mi amor Buenos dias jsjsj FELIZ ANIVERSARIOOOOO jsjsjs como estas mi vida? todo bien? te extraño :( no estar contigo me llena de pena amor.\n\nEspero seguir cumpliendo màs meses con usted preciosa con la chica que eleji para toda mi vida.\n\nMi amor por ti es como una cascada nunca para :3.\n\n Te amo hasta el infinito y màs alla.`;  } else {
...
function showSignature() {
  ...
  signature.textContent = firma ? decodeURIComponent(firma) : "ATT: TU BROCOLI:3 ";
...
function showCountdown() {
  ...
  let eventDate = eventParam ? new Date(eventParam + 'T00:00:00') : new Date('2025-08-23T00:00:00');
  ...
  function update() {
    ...
    container.innerHTML =
      `El tiempo de novios: <b>${days}</b> días<br>` +
      `El siguiente Aniversario es: <b>${eventDays}d ${eventHours}h ${eventMinutes}m ${eventSeconds}s</b>`;
...
function playBackgroundMusic() {
  ...
  if (youtubeParam) {
    ...
    if (!helpMsg) {
      ...
      helpMsg.style.background = 'rgba(8, 5, 5, 0.95)';
